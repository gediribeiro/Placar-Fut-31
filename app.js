// ============================================================================
// PLACAR FUT 31 – APP.JS v1.2.0 (CORRIGIDO)
// ============================================================================
const APP_VERSION = 'v1.2.0'; // AGORA NO ESCOPO GLOBAL
...
const PlacarApp = (function() {
  ...
  async function iniciar() {
    ...
    document.getElementById('btnIniciar')?.classList.add('btn-animating-green');
    setTimeout(() => document.getElementById('btnIniciar')?.classList.remove('btn-animating-green'),600);
  ...
  async function fim() {
    if (!state.partida) return showToast('Inicie o jogo!','error');
    document.getElementById('btnFim')?.classList.add('btn-animating-red');
    setTimeout(() => document.getElementById('btnFim')?.classList.remove('btn-animating-red'),600);
    clearInterval(state.timer); state.timer = null;
  ...
  function aumentarGol(time) {
    ...
    if (titulo) titulo.textContent = `⚽ Gol do ${time==='A'?state.nomeA:state.nomeB}! Quem fez?`; // ✅ CORRIGIDO
  ...
  // ==========================================================================
  // 10. CONTROLE DE FALTAS (CORRIGIDO)
  // ==========================================================================
  function registrarFalta(time) {
    if (!state.partida) return showToast('Inicie o jogo!','error');
    fecharPopup(); fecharPopupRemover(); fecharPopupNome();
    state.timeAtualFalta = time;
    const titulo = document.getElementById('popupTituloFalta');
    if (titulo) titulo.textContent = `⚠️ Falta do ${time==='A'?state.nomeA:state.nomeB}. Quem fez?`; // ✅ CORRIGIDO
...
function iniciarAppComSplash() {
  ...
  function inicializarAppPrincipal() { PlacarApp.init(); exibirVersao(); initTutorial(); } // ✅ CHAMADA CORRETA AQUI

  const tempoMinimoSplash = new Promise(resolve => setTimeout(resolve, 2000));

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      tempoMinimoSplash.then(() => { esconderSplash(); inicializarAppPrincipal(); });
    });
  } else {
    tempoMinimoSplash.then(() => { esconderSplash(); inicializarAppPrincipal(); });
  }
